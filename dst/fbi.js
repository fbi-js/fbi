...
function install(pkgs, rootPath, command, opts) {
  var prevDir = process.cwd();
  // log(`Installing ${command} packages : ${pkgs.join(' ')} ...`)
  process.chdir(rootPath);
  var cmd = command + ' install ' + pkgs.join(' ') + ' ' + (opts ? opts : '');
  log(cmd + '...');
  return new Promise(function (resolve, reject) {
  ...
  createClass(Cli, [{
    ...
    value: function task() {
      ...
      return Promise.resolve().then(function () {
        if (!!_this7.next) {
          return Promise.resolve().then(function () {
            return function () {
              var dbTasks, pathTasks, existUserTasks, source, parser, deps, needinstall;
              return Promise.resolve().then(function () {
                dbTasks = new Store('tasks');
                pathTasks = cwd('./fbi/tasks.js');
                return exist$1(pathTasks);
              }).then(function (_resp) {
                existUserTasks = _resp;
                return read$1(pathTasks);
              }).then(function (_resp) {
                source = _resp;
                ...
                if (needinstall.length) {
                  return install(needinstall, process.cwd(), 'npm', '--save-dev --registry=https://registry.npm.taobao.org');
                }
              }).then(function () {});
            }();
          }).catch(function (e) {
    ...
    value: function run() {
      ...
      try {
        cmds.map(function (cmd) {
          if (_this2.tasks[cmd]) {
            _this2.tasks[cmd].fn.call(_this2);
          }
        });
      } catch (e) {
    ...
    value: function makeConfig() {
      ...
      return Promise.resolve().then(function () {
        return Promise.resolve().then(function () {
          // access user config
          _path = _.cwd(_this14.config.paths.options);
          return exist$1(_path);
        }).then(function (_resp) {
          _this14.isFbi = _resp;
          if (_this14.isFbi) {
            usrCfg = require(_path);

            _.merge(_this14.config, usrCfg);
          }
        }).catch(function (e) {
          _.log(e);
        });
