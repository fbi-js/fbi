  ...
  sources.forEach(function(source) {
    for (var p in source) {
      if (typeof source[p] === 'object') {
        if (
          (target[p] === null || target[p] === undefined) &&
          (source[p] === null || source[p] === undefined)
        ) {
          target[p] = null
        } else {
...
function parseArgvs(arr, prefix) {
  ...
  arr.reduce((prev, curr, idx) => {
    if (curr.indexOf(prefix) === 0) {
      ...
      curr.indexOf('/') >= 0 ||
      ['.js', '.json'].includes(path.extname(curr))
    ) {
      // not a task name
      if (ret[prev]) {
        if (Array.isArray(ret[prev]['params'])) {
            ...
            case 'template':
              {
                ...
                //             tasksTxt += `
                // ${style.green(
                //   fillGap((item.alias ? item.alias + ', ' : '') + item.name, 15, ' ')
                // )}`
                //           } else {
                tasksTxt += `
      ${fillGap(
...
//       tmplsTxt += `
//       ${current}  ${style.green(item.name)} ${style.blue(
//         'v' + item.version
//       )} ${item.desc ? ' - ' + item.desc : ''}`
//     })
//   }
...
function genTmplHelpTxt(all, curr) {
  let txt = `
  ...
  if (all && Object.keys(all).length) {
    Object.keys(all).map(item => {
      const star = `${item === (curr ? curr.name : '') ? style.yellow('★') : '★'}`
      const version = item === (curr ? curr.name : '') ? curr.version : all[item].version

      tmplsTxt += `
      ${star}  ${style.green(item)} ${style.blue(version)} ${all[
        item
      ].description
  ...
  if (!tmplsTxt) {
    tmplsTxt = style.grey(`
      No template, use 'fbi clone <repo>' or 'fbi atm' to add templates.`)
  } else {
    tmplsTxt =
      style.grey(`
      usage: fbi init template
...
function repoClone(url, cwd, name) {
  return new Promise((resolve, reject) => {
    const cmd = spawn('git', ['clone', url], {
      cwd,
    ...
    cmd.on('close', status => {
      if (status === 0) {
        ...
        reject(`Error with status ${status}.`)
      }
    })
  })
}

function repoPull(repo) {
  return new Promise((resolve, reject) => {
    const cmdReset = spawn('git', ['reset', '--hard'], {
      cwd: repo,
      stdio: [0, 1, 2]
    })

    cmdReset.on('close', status => {
      if (status === 0) {
...
function repoVersion(repo, lines) {
  return new Promise((resolve, reject) => {
    ...
    //   reject(data.toString().replace(/(\r|\n)/g, ''))
    // })
...
function repoCheckout(repo, version) {
  const v = version || 'master'
  return new Promise((resolve, reject) => {
    const cmd = spawn('git', ['checkout', v], {
      cwd: repo
    })
