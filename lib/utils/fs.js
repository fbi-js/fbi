...
function write(_path, content, ...options) {
  ...
  return mkdirp(path.dirname(_path).split(path.sep)).then(() => writeFile(_path, content, writeOptions))
}
 ...
 * @returns
 */
async function copy({from, to, ignore = [], quiet = false, log} = {}) {
  ...
  // T if (path.extname(to)) {
  //   throw new Error('Target should be a directory.')
  // }
...
async function _copy({source, target, quiet, logger, srcRoot, dstRoot} = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    wr.on('finish', () => {
      if (!quiet) {
        logger(`Copy \`${path.relative(srcRoot, source)}\` => \`${path.relative(dstRoot, target)}\``)
      }
      resolve()
    })
    rd.pipe(wr)
...
function list(dir, ignore = [], depth) {
  ...
  return stat(dir).then(stats => {
    if (stats && stats.isDirectory()) {
      return _list(dir, ignore)
        .then(file => Promise.all(file.map(item => list(path.resolve(dir, item), ignore))))
...
function _list(dir, ignore) {
  ...
  return readDir(dir).then(ret => (ignore && ignore.length ? ret.filter(valid) : ret), err => err)
}
