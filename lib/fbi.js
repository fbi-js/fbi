...
module.exports = class Fbi {
  ...
  async install() {
    ...
    if (opts.template) {
      try {
        const _path = _.join(opts.DATA_TEMPLATES, opts.template, 'package.json')
    ...
    const targets = [{
      ...
      name: 'Template',
      deps: tmplDeps,
      path: _.join(opts.DATA_TEMPLATES, opts.template || ''),
  ...
  async update() {
    const opts = this.options
    if (opts.template) {
      const pkgPath = _.join(opts.DATA_TEMPLATES, opts.template, 'package.json')
      if (await _.exist(pkgPath)) {} else {
        _.log(`Error: Global template "${opts.template}" not exist`, -2)
        process.exit(1)
      }
