...
module.exports = class Template {
  constructor ({ mode, store, stores, configs, options, logger } = {}) {
    this.mode = mode
    this.store = store
    this.stores = stores
  ...
  async init (tmplInfo, target, extra) {
    ...
    if (!await utils.fs.isEmptyDir(target.dir)) {
      const answer = await utils.flow.prompt(
        `${this.logger.getPrefix()}${utils.style.yellow('Warning: `' + pathRelative + '` is not empty, sure you want to cover it? (y/N):')}`,
        'yellow'
      )
      if (answer !== 'y') {
        return {
    ...
    if (tmplInfo.version) {
      ...
      try {
        versionChanged = await version.change({
          dir: tmplInfo.path,
    ...
    if (withOptions) {
      ...
      if (await utils.fs.exist(optsSrc)) {
        await utils.fs.copy({
          from: optsSrc,
          to: optsDst,
          log: this.logger
  ...
  async update (dstFolder, opt) {
    dstFolder = dstFolder || ''
    const tmplName = this.options.template ? this.options.template.name : ''
    if (!tmplName) {
      return
    }
    ...
    const projName = dstFolder
      ? path.basename(dstFolder)
      : path.basename(process.cwd())

    this.logger.info(
      `Updating project \`${projName}\`${opt ? ' ' + opt : ''}... `
    )
    ...
    switch (opt) {
      ...
      case 'with-all': {
        ...
        const pkgTmplPath = path.join(tmplPath, 'package.json')
        let pkg = {}
        if (await utils.fs.exist(pkgTmplPath)) {
          const pkgTmpl = require(pkgTmplPath)
          delete pkgTmpl.dependencies
          pkg = utils.assign({}, pkgTmpl, pkgLocal)
        }
  ...
  async createPackageJson (src, dst, projectName, templateInfo) {
    ...
    delete projectPackage.fbi
    delete projectPackage.keywords
    delete projectPackage.repository
    delete projectPackage.engines
    delete projectPackage.bugs
    delete projectPackage.homepage
    const localPackage = utils.assign(projectPackage, {
      name: projectName,
      version: '0.0.0',
      description: `A project base on '${templateInfo.name}' template`,
      author: '',
      repository: {},
      fbi: {
