...
module.exports = class Conifg {
  ...
  async baseConfig() {
    ...
    const dataRoot = path.join(process.env.HOME, this.configs._DATA_ROOT)
  ...
  async userConfig(mode, stores, logger) {
    // Options: local
    const localOpts = await this.local(stores)
    ...
    if (mode.template && typeof mode.template === 'string') {
      ...
      templateName = template.nameParse(
        mode.template,
    ...
    const options = utils.assign(templateOpts, localOpts)
  ...
  async local(stores) {
    ...
    if (!localCfg.template || !localCfg.template.name) {
      ...
      // Name parse
      localCfg.template.name = template.nameParse(
        localCfg.template.name,
        this.configs.TEMPLATE_PREFIX
      )
      // const tmplInfo = stores[localCfg.template.name]
      // if (tmplInfo) {
      //   // Version fill
      //   localCfg.template.version =
      //     localCfg.template.version || tmplInfo.version.latest
      // } else {
      //   delete localCfg.template
      // }
    }
    ...
    // localCfg.template = localCfg.template || ''

    // if (
    //   localCfg.template.name &&
    //   !localCfg.template.name.startsWith(this.configs.TEMPLATE_PREFIX)
    // ) {
    //   localCfg.template.name =
    //     this.configs.TEMPLATE_PREFIX + localCfg.template.name
    // }
  ...
  async template(templatePath) {
    ...
    if (await utils.fs.exist(templatePath)) {
      ...
      // tmplCfg.template = stores[templateName]
    ...
    if (templateName && (await utils.fs.exist(this.configs._STORE_FILE))) {
      const stores = require(this.configs._STORE_FILE)
      if (!stores || Object.keys(stores).length < 1) {
        throw `Template \`${templateName}\` not found.`
      }

      if (stores && Object.keys(stores).length > 0) {
        const templateInfo = stores[templateName]
        if (templateInfo) {
          const templatePath = stores[templateName].path
          if (await utils.fs.exist(templatePath)) {
            // Deprecated
            tmplCfg.node_modules_path = path.join(templatePath, 'node_modules')
            tmplCfg.template = stores[templateName]
            ...
            // Config file: fbi/config.js
            const tmplCfgFile = path.join(
              templatePath,
              this.configs.TEMPLATE_CONFIG
            )
            if (await utils.fs.exist(tmplCfgFile)) {
              utils.assign(tmplCfg, require(tmplCfgFile))
            }
          }
        }
      }
