...
module.exports = class Task {
  ...
  async run (task, ctx) {
    ...
    ctx.logger.info(
      `Running ${taskInfo.type} task ${utils.style.bold(taskInfo.name)}${taskInfo.tmpl ? ' in ' + taskInfo.tmpl : ''}${taskInfo.tmplVer ? '@' + taskInfo.tmplVer + '`' : ''}...`
    )
    ...
    if (exist) {
      ...
      lists.map(t => {
        if (utils.type.isTaskFile(t)) {
          ...
          const info = options.tasks && options.tasks[name]
            ? options.tasks[name]
            : ''
          tasks.push({
            name,
