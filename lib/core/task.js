  ...
  async run(task, ctx) {
    ...
    ctx.logger.info(
      `Running ${taskInfo.type} task ${utils.style.bold(taskInfo.name)}${
        taskInfo.tmpl ? ' in ' + taskInfo.tmpl :
        ''}${
        taskInfo.tmplVer ? '@' + taskInfo.tmplVer + '`' :
        ''}...`
    )
    ...
    try {
      ...
      if (err && err.message && err.message.indexOf('Cannot find module') >= 0) {
        ctx.logger.error(err.message)
        ctx.logger.warn('Run `fbi i` to install dependencies')
      } else {
    ...
    if (type === 'local') {
      ...
      if (
        ...
        try {
          ...
          const mainFilePath =
            mainFile ? utils.path.cwd(mainFile) :
            ''
          const mainFileExist =
            mainFilePath ? await utils.fs.exist(mainFilePath) :
            false
          if (mainFileExist) {
