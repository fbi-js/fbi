...
class Fbi extends EventEmitter {
    ...
    if (this.options.template) {
      ...
      if (tmplInfo && tmplInfo.version) {
        ...
        try {
          ...
          if (versionChanged) {
            this.logger.info(`Change template's version to ${target.version}`)
        ...
        this.logger.error(
          ...
          }' not found locally. Please add the template first.`
        )
        this.logger.info('Official templates: https://github.com/fbi-templates')
        process.exit(0)
      }
    }
    ...
    for (const url of params) {
      try {
        ...
        if (
          ...
          this.options.template.name === tmplInfo.fullname
        ) {
          // change to current project's template version
          const versionChanged = await this.version.change({
            dir: tmplInfo.path,
            name: tmplInfo.fullname,
            version: this.options.template.version,
            showlog: false,
            logger: this.logger,
            store: this.store
          })
          ...
          if (versionChanged) {
            tmplInfo.version.current = this.options.template.version
          }
        }
        ...
        if (info.version && info.version.current) {
          this.logger.log(`Current version '${info.version.current}'`)
        }
  ...
  async init (cmd, params, opts) {
    ...
    this.logger.info(
      `Creating project \`${projectName}\` via \`${tmplInfo.name}${
    ...
    Promise.all(
      params.map(async item => {
        try {
          ...
          // Set store
          const latestVersion = await this.version.getLatestVersion(
            itemInfo.path
          )
  ...
  async install (cmd, params, opts, targetDir) {
    ...
    for (const item of targets) {
      try {
        ...
        if (needInstall) {
          this.logger.log(
            `Installing ${item.type} dependencies${
    ...
    `
      ...
      ;['global', 'template', 'local'].map(type => {
        ...
        tasks[type].map(t => {
          ...
          const center = flag || version ? (flag + ' ' + version).padEnd(9) : ' '.padEnd(19)
          const right = t.desc || ''
      ...
      switch (param) {
        ...
        case 'stores': {
          ...
          for (const c of Object.keys(stores)) {
            txt += `\n${utils.style.cyan(c)}\n`
            txt += `${'type'.padStart(15)}: ${
              stores[c].type
            }\n${'path'.padStart(15)}: ${stores[c].path}\n${'version'.padStart(
              15
            )}: ${JSON.stringify(stores[c].version)}\n${'repository'.padStart(
              15
            )}: ${stores[c].repo}\n${'description'.padStart(15)}: ${
