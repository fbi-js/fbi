...
class Fbi extends EventEmitter {
      ...
      params.map(async url => {
        try {
          ...
          if (!template.isTemplate(repoName)) {
            this.logger.error(
              `FBI do not support this template, the name should start with \`${this
          ...
          this.logger.success(`\`${repoName || url}\` added.`)
  ...
  async init(cmd, params, opts) {
    ...
    if (this.utils.type.isGitUrl(pathObj.name)) {
      ...
      fullname = template.nameParse(pathObj.name, [
    ...
    const initSuccess = await template.init(
      tmplInfo,
      {
        dir: targetDir,
        name: projectName,
    ...
    Promise.all(
      params.map(async item => {
        try {
          ...
          const updated = await version.update(itemInfo.path, this.logger)
          if (!updated) {
            return this.logger.error(
