...
class Fbi extends EventEmitter {
  ...
  async init(cmd, params, opts) {
    ...
    const extra =
        opts.o || opts.option ? 'with-options' :
        opts.t || opts.task ? 'with-tasks' :
    ...
    if (tmplInfo.version) {
      ...
      this.logger.warn(`Template \`${tmplInfo.name}\` not support version control.`)
      pathObj.version = ''
    }
    this.logger.info(
      `Creating project \`${projectName}\` via \`${tmplInfo.name}${pathObj.version ? '@' + pathObj.version : ''}\`${matchOption ? ' ' + matchOption : ''}...`
    )
    ...
    if (tmplInfo.version && pathObj.version) {
      ...
      await this.store.update(tmplInfo.fullname, {
        version: Object.assign(this.stores[tmplInfo.fullname].version, {
    ...
    Promise.all(
      params.map(async item => {
        try {
          ...
          await this.version.change(itemInfo.path, latestVersion)
  ...
  async use(cmd, params) {
    ...
    if (versionChanged) {
      ...
      this.logger.warn(`Template already in version \`${ver}\`.`)
    }
  ...
  async install(cmd, params, opts, targetDir) {
    ...
    for (const item of targets) {
      try {
        ...
        if (needInstall) {
          this.logger.log(`Installing ${item.type} dependencies${packages ? ' ' + packages.join(' ') : ''}...`)
        ...
        params.map(p => {
          ...
          if (key && val) {
            if (canSetItems.includes(key)) {
              ...
              throw `Configuration item \`${key}\` is readonly or not exist.`
            }
          } else {
    ...
    `
      ...
      ;['global', 'template', 'local'].map(type => {
        const flag =
            ...
            ''
        const color =
            type === 'local' ? highlightColor :
            ''
        tasks[type].map(t => {
          const version =
            t.version ? utils.style.grey(t.version) :
            ''
          const alias =
            t.alias ? t.alias + ', ' :
            ''
          ...
          const left = (
            color ? utils.style[color](`${alias}${name}`) :
            `${alias}${name}`).padEnd(17)
          const center =

              flag || version ? (flag + ' ' + version).padEnd(9) :
              ' '.padEnd(18)
          const right = t.desc || ''
      ...
      if (this.stores) {
        Object.keys(this.stores).map(item => {
          if (item.startsWith(this.configs.TEMPLATE_PREFIX)) {
            ...
            const star =
              isCurrent ? utils.style[highlightColor]('★  ') :
              '★  '
            const name =
              isCurrent ? utils.style[highlightColor](_name.padEnd(15)) :
              _name.padEnd(15)
            const version =
              this.stores[item].version ? utils.style.grey(
